...
class GeneratorFullModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if (
            self.loss_weights["equivariance_value"]
            + self.loss_weights["equivariance_jacobian"]
        ) != 0:
            transform = Transform(
                x["driving"].shape[0], **self.train_params["transform_params"]
            )
            ...
            if self.loss_weights["equivariance_value"] != 0:
                value = torch.abs(
                    kp_driving["value"]
                    - transform.warp_coordinates(transformed_kp["value"])
                ).mean()
            ...
            if self.loss_weights["equivariance_jacobian"] != 0:
                ...
                loss_values["equivariance_jacobian"] = (
                    self.loss_weights["equivariance_jacobian"] * value
                )
